...
function CategoryForm({ type, editId, onClose }: { type: 'expense' | 'earning'; editId: string | null; onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!name.trim()) return;
    onClose(); // Close modal immediately for instant UI feedback
...
export function CategoriesTab() {
  ...
  const expenseCategories = useFirestoreQuery<ExpenseCategory>('expenseCategories') || [];
  const earningCategories = useFirestoreQuery<EarningCategory>('earningCategories') || [];
