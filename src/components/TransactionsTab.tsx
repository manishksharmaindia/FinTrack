...
import { Search, Filter, Calendar, ArrowUpRight, ArrowDownRight, ChevronLeft, ChevronRight, Trash2, Pencil, Mic } from 'lucide-react';
import { db } from '../db';
import { VoiceTransactionAssistant } from './VoiceTransactionAssistant';
import type { Expense, Earning, ExpenseCategory, EarningCategory } from '../db';
import { useAppStore } from '../store';
import { getIcon, formatCurrency, formatDate, paymentMethods, months, years } from '../utils';
import { Modal } from './Modal';

function ExpenseForm({ editId, onClose }: { editId: string | null; onClose: () => void }) {

  const cats = useFirestoreQuery<ExpenseCategory>('expenseCategories') || [];
...
function EarningForm({ editId, onClose }: { editId: string | null; onClose: () => void }) {

  const cats = useFirestoreQuery<EarningCategory>('earningCategories') || [];
  const [date, setDate] = useState(() => {
    const d = new Date();
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
  });
...
export function TransactionsTab() {
  ...
  const allExpenses = useFirestoreQuery<Expense>('expenses') || [];
  const allEarnings = useFirestoreQuery<Earning>('earnings') || [];
  const expCats = useFirestoreQuery<ExpenseCategory>('expenseCategories') || [];
  const earnCats = useFirestoreQuery<EarningCategory>('earningCategories') || [];

  const expensesRaw = useMemo(() => allExpenses.filter(e => e.date >= startDate && e.date <= endDate), [allExpenses, startDate, endDate]);
  const earningsRaw = useMemo(() => allEarnings.filter(e => e.date >= startDate && e.date <= endDate), [allEarnings, startDate, endDate]);
  ...
  type TxItem = { id: string; type: 'expense' | 'earning'; date: string; amount: number; catName: string; catIcon: string; catColor: string; notes: string; paymentMethod?: string };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto shrink-0">
            ...
            <button onClick={() => setShowVoiceAssistant(true)}
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-600 text-white text-sm font-medium shadow-md hover:shadow-lg transition-all whitespace-nowrap">
              <Mic size={16} /> Voice
            </button>
          </div>
        </div>
