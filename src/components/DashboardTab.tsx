...
import type { Expense, Earning, ExpenseCategory } from '../db';
...
export function DashboardTab() {
  ...
  const categoryPieData = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([id, amount]) => ({ name: expCatMap[id]?.name || 'Unknown', value: amount, color: expCatMap[id]?.color || '#64748b' }))
  ...
  const topCategories = useMemo(() => {
    ...
    return Object.entries(map)
      .map(([id, amount]) => {
        const cat = expCatMap[id];
        return { id: id, name: cat?.name || 'Unknown', icon: cat?.icon || 'MoreHorizontal', color: cat?.color || '#64748b', amount, budget: cat?.monthlyBudget || 0 };
      })
